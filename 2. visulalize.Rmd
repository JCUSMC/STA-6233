...
<!-- Not useful since you don't know how many vehicles in each category -->
...
ggplot(data = mpg, mapping = aes(x = displ, y = hwy)) +
  geom_point() + 
  geom_point(data = mpg[mpg$displ > 5&mpg$hwy > 20,], colour = "red", size = 2.2)
...
<!-- Area Chart -->
```{r, message = FALSE}
  ggplot(data = mpg, aes(x = year, y = cyl, fill = drv)) + 
  geom_area()
```
...
You could use this method to build _any_ plot that you imagine. In other words,
you can use the code template that you've learned in this chapter to build hundreds
of thousands of unique plots.
